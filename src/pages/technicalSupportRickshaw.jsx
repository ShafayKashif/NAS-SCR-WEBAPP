...
function TechnicalSupportRickshaw() {
  const navigate = useNavigate();
  function createData(email, name, plate, phone, problemType, problemDetails, resolve) {
    return { email, name, plate, phone, problemType, problemDetails, resolve };
  }
  ...
  const fetchObtainedState = async () => {
    try {
      ...
      const obtainedTechSupport= await getRecords("Technical Support Driver")
  ...
  const handleSubmit=(id, obj)=>{
    ...
    updateRecord('Technical Support Driver', id, newRecordData)
  ...
  const createRows=()=>{
    ...
    techSupportInfo.forEach((user)=>{
        ...
        let name=obj.name
        let phone=obj.phone
        myRows.push(createData(user.email, name, user.assigned, phone, user['Problem Type'], user['Problem Detail'], (user['Resolved']?<div>Resolved</div>:<a onClick={()=>handleSubmit(user.id, user)} style={{ textDecorationLine: 'underline', color: '#58AA42' }}>Mark as Resolved</a>)))
    })
    setRows(myRows)
  }
  ...
  return (
    ...
    <Homepage />
    <h1 className="heading">Technical Support Rickshaw</h1>
...
export default TechnicalSupportRickshaw;
